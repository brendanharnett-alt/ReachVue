...
export default function ContactsTable() {
  ...
  const [groupBy, setGroupBy] = useState("none") // "none" | "company" | "tag"
  const [expandedGroups, setExpandedGroups] = useState({})
  const [selectedContact, setSelectedContact] = useState(null)

  // modal state
  const [activeDialog, setActiveDialog] = useState(null) // "email" | "call" | "linkedin" | "touch" | "history" | null
    ...
    if (type === "history") {
      ...
      try {
        ...
        if (res.ok) {
          ...
          console.error("Failed to fetch touch history")
          setTouchHistory([])
        }
  ...
  // 🔹 Inline updater when a touch is logged
