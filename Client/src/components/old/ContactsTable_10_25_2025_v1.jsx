...
export default function ContactsTable() {
  ...
  const [filters, setFilters] = useState({
    ...
    title: [],
    tag: [],
    // lastTouch now uses a config object via LastTouchFilterModal (null = inactive)
    lastTouch: null,
  })
  const [searchTerm, setSearchTerm] = useState("")
  // NEW: Tag filter modal + mode
  const [showTagFilter, setShowTagFilter] = useState(false)
  const [tagFilterMode, setTagFilterMode] = useState("is")
  // NEW: Last Touch modal
  ...
  const clearFilters = () => {
    ...
    setSearchTerm("")
    setTagFilterMode("is") // reset tag filter mode too
  }
  ...
  // (kept for legacy display if needed; not used by the new modal)
  ...
  // 🔹 Helpers: Tag filter + Last Touch filter logic
  function applyTagFilter(contact, selectedTags, mode) {
    const tags = contact.tags?.map((t) => t.tag_name) || []
    switch (mode) {
      case "is":        return selectedTags.length === 0 || tags.some((t) => selectedTags.includes(t))
      case "isNot":     return selectedTags.length === 0 || !tags.some((t) => selectedTags.includes(t))
      case "empty":     return tags.length === 0
      case "notEmpty":  return tags.length > 0
      default:          return true
    }
  }
  ...
  function isLastTouchActive(cfg) {
    if (!cfg) return false
    if (Array.isArray(cfg)) return cfg.length > 0 // backward compatibility
    // config object
    return !!cfg.type && cfg.type !== "none"
  }

  function matchLastTouch(contact, cfg) {
    // Backward compatibility: legacy array of formatted dates
    if (Array.isArray(cfg)) {
      const formatted = contact.last_touched_at
    ...
    const hasValue = !!iso
    const touched = hasValue ? new Date(iso) : null
    ...
    switch (cfg.type) {
      ...
      case "relative": {
        ...
        // expect cfg.relative like '7d', '14d', '30d', '90d'
        const rel = (cfg.relative || "").toLowerCase()
        const n = parseInt(rel, 10)
        if (!n || !rel.endsWith("d")) return true // if malformed, don't filter out
      ...
      case "between": {
        ...
        if (start && end) return touched >= startOfDay(start) && touched <= endOfDay(end)
  ...
  const filterValuesMap = {
    ...
    title: uniqueTitles,
    tag: uniqueTags,
    // lastTouch no longer uses discrete values; kept for API parity
    lastTouch: uniqueLastTouches,
  }

  // NEW: open the right filter modal
  const handleOpenFilter = (key) => {
    if (key === "tag") {
      setShowTagFilter(true)
    } else if (key === "lastTouch") {
      setShowLastTouchFilter(true)
    } else {
    ...
    <div className="rounded-xl border bg-white p-4 shadow-sm">
      ...
      <div className="border bg-gray-50 rounded-lg p-3 mt-2 mb-4 flex justify-between items-center">
        ...
        <div className="flex flex-wrap gap-2 justify-end">
          {[
            { key: "name", icon: <UserIcon className="h-4 w-4" />, label: "Name", active: filters.name.length },
            { key: "company", icon: <BuildingIcon className="h-4 w-4" />, label: "Company", active: filters.company.length },
            { key: "title", icon: <BriefcaseIcon className="h-4 w-4" />, label: "Title", active: filters.title.length },
            { key: "tag", icon: <TagIcon className="h-4 w-4" />, label: "Tag", active: filters.tag.length || tagFilterMode !== "is" },
            { key: "lastTouch", icon: <ClockIcon className="h-4 w-4" />, label: "Last Touch", active: isLastTouchActive(filters.lastTouch) },
          ].map((f) => (
            <Button
              key={f.key}
      ...
      {/* Shared Filter Modal (for all EXCEPT Tag and Last Touch) */}
      ...
      {/* ✅ NEW: Last Touch Filter Modal */}
      <LastTouchFilterModal
        open={showLastTouchFilter}
        onOpenChange={setShowLastTouchFilter}
        // Provide current selection for editing
        selected={filters.lastTouch}
        onApply={(config) => {
          // Expecting shape like:
          // { type: 'relative'|'before'|'after'|'between'|'empty'|'notEmpty', relative?: '7d', start?: 'YYYY-MM-DD', end?: 'YYYY-MM-DD' }
