...
export default function ContactsTable() {
  ...
  // ✅ Update last touch for specific contact (used by EmailModal, CallModal, etc.)
    ...
    <div className="rounded-xl border bg-white p-4 shadow-sm">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-2">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" onClick={() => setShowTagModal(true)}>Manage Tags</Button>
        ...
        <div className="flex gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              ...
              <DropdownMenuItem onClick={() => setGroupBy("company")}>Company</DropdownMenuItem>
              <DropdownMenuItem onClick={() => setGroupBy("tag")}>Tags</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      ...
      {/* Table */}
      <div className="flex">
        <div className={`relative h-[66vh] overflow-auto pb-5 ${selectedContact ? "w-2/3" : "w-full"}`}>
          <table className="table-auto text-sm w-full border-collapse">
            <thead className="bg-muted sticky top-0 z-20">
              <tr className="text-left text-xs">
                <th className="p-2 font-medium"><input type="checkbox" className="w-5 h-5 cursor-pointer" /></th>
              ...
              {Object.entries(groupedContacts).map(([group, rows]) => (
                <React.Fragment key={group}>
                  {groupBy !== "none" && (
                    ...
                    >
                      <td className="p-2">
                        {expandedGroups[group] ? <ChevronDown className="h-4 w-4 inline" /> : <ChevronRight className="h-4 w-4 inline" />}
                      </td>
                      <td className="p-2 text-blue-700" colSpan={1}>
                        {group} <span className="text-gray-500 text-sm">({rows.length})</span>
                      </td>
                  ...
                  {(groupBy === "none" || expandedGroups[group]) &&
                    rows.map((contact) => (
                      <tr key={contact.id} className="border-t">
                        ...
                        <td className="p-2 truncate max-w-[140px]">{contact.company}</td>
                        <td
                        ...
                        <td className="p-2 truncate max-w-[160px]">{contact.title}</td>
      ...
      <EmailModal
        ...
        onClose={closeDialog}
        onSend={({ contactId, lastTouched }) => updateLastTouch(contactId, lastTouched)}
      />
      <CallModal
