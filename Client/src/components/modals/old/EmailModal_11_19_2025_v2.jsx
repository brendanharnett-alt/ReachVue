...
export default function EmailModal({ open, contact, onClose, onSend }) {
  ...
  const handleSend = async () => {
    ...
    // ⭐ NEW — ALWAYS create a brand-new thread for new emails
    const thread_id = uuidv4()
    const parent_touch_id = null

    try {
      // Send to Chrome extension
      if (!window || typeof window.postMessage !== "function") {
        alert("⚠️ Unable to send email — messaging not available.")
        return
      }
      ...
      // ⭐ UPDATED: Log touch with thread metadata
      await logTouch({
