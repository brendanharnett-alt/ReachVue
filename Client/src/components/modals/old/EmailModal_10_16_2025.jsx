...
export default function EmailModal({ open, contact, onClose, onSend }) {
  ...
  const handleSend = () => {
    ...
    // Optional: local callback (for in-app logging)
    onSend?.({ to: recipient, subject, body: bodyHtml })

    // ✅ Send to Outlook bridge
    try {
      window.postMessage(
        {
      ...
      console.log("📤 Posted message to Outlook bridge:", {
        recipient,
        subject,
        bodyHtml,
      })
    } catch (err) {
      console.error("💥 Failed to post message to Outlook bridge:", err)
    }
