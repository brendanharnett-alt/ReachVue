...
export default function EmailModal({ open, contact, onClose, onSend }) {
  ...
  // ✅ Updated send handler with safety check
  const handleSend = () => {
    const subject = subjectRef.current?.value || ""
    let bodyHtml = editor?.getHTML() || ""
    const recipient = contact.email

    // Normalize blank paragraphs (Tiptap creates <p></p> for empty lines)
    bodyHtml = bodyHtml.replace(/<p><\/p>/g, "<div>&nbsp;</div>")

    // Local callback (for in-app logging)
    onSend?.({ to: recipient, subject, body: bodyHtml })

    try {
      // ✅ Safety: ensure postMessage exists before sending
      if (!window || typeof window.postMessage !== "function") {
        alert("⚠️ Unable to send email — messaging not available in this browser.")
        return
      }

      console.log("📤 Posting message to ReachVue extension bridge:", {
        recipient,
        subject,
      })

      // ✅ Use postMessage so extension content script can pick it up
      window.postMessage(
        {
      ...
      alert("⚠️ Something went wrong trying to reach the extension.")
    }
